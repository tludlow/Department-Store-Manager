...
ALTER TABLE inventory ADD CONSTRAINT positive_stock_check CHECK (ProductStockAmount >= 0);
...
ALTER TABLE orders ADD CONSTRAINT valid_order_type_check CHECK (OrderType IN ('InStore', 'Collection', 'Delivery'));
ALTER TABLE orders ADD CONSTRAINT valid_order_completed_Value_check CHECK (OrderCompleted IN ('0', '1'));

-- TODO MAKE A TRIGGER THAT DETERMINES WHETHER OR NOT THE ORDER IS COMPLETED OR NOT!
...
-- You wouldn't want to order a 0 amount, or negative amount of a product, you just wouldnt include it within this table.
ALTER TABLE order_products ADD CONSTRAINT positive_order_amount_check CHECK (ProductQuantity > 0);
...
CREATE OR REPLACE PROCEDURE create_order (order_type IN varchar2, order_completed IN integer, order_placed IN Date, order_lines IN order_items) AS
...
-- CREATE OR REPLACE TRIGGER decrement_stick
...
-- Trigger to check that the amount being ordered is at most the amount in the inventory  (amount in stock)
-- CREATE OR REPLACE TRIGGER order_products_check_quantity_in_inventory
-- AFTER INSERT OR UPDATE ON order_products
-- FOR EACH ROW
-- DECLARE
-- 	in_stock inventory.ProductStockAmount%type;
-- BEGIN
...
-- IF (:NEW.ProductQuantity>=in_stock) THEN
